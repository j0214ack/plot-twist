...
4. **前端開發／觀測工具**：讓開發者能在 OpenAB 直接開發前端、取得 public URL，並看到 log 或 devtools 資訊。這是選題時提出的另一個獨立專案選項，不是敘事遊戲的子系統。
...
| **C. OpenAB 前端 DevTool** | 前端工程師在 OpenAB 開發畫面，取得 public URL、log、console、network 等資訊 | **獨立專案選項**；不是 A 或 B 的底層、功能或未來 roadmap |
...
OpenAB DevTool 不包含在這條路線中。如果團隊選擇 OpenAB 題目，應另外為它建立獨立的問題定義、使用者流程與 Demo 計畫。

> 第 5–13 節皆以「團隊選擇候選 A」為前提，只是該方向的 POC 草案，不適用於 OpenAB DevTool。
...
- 加入與這條敘事體驗無關的 OpenAB DevTool 功能。
