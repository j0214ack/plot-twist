...
一次 utterance 先被編譯成一個 `SpellBundle`。Bundle 可以包含一到多個有順序或依賴關係的 modules；每個 module 載入後形成一個 `SpellArtifact`。

```ts
interface SpellBundle {
  summary: string;
  modules: Array<{
    id: string;
    label: string;
...
interface LocomotionCapability {
...
Effect 是 module-owned resource：別的 module 可以查詢，但不能直接移除；owner dispose、setup failure 或 bundle rollback 時由 Host 清除。每個 `SpellArtifact` 以 `effectIds` 暴露自己建立的 effects，供 dependent module 透過 public capability 取得 actor 與 mode。`flight` 不改寫 navigation 的 solid collision policy。
...
`source` 的 runtime ABI 是一個回傳 `MechanicModule` 的 JavaScript factory expression：

```ts
type MechanicModuleFactory = (
  dependencies: Readonly<Record<string, SpellArtifact>>,
) => MechanicModule;
```

Factory 只能由 dependency bindings 取得前面 modules 的 artifacts；它要等到 `setup(context)` 才能操作 public Game SDK。Bundle executor 必須依序 load，且其中一步失敗時反向 dispose 本次 bundle 已載入的所有 modules。完整格式與限制見 Decision 0002 的 Generated source ABI。
...
Reference harness 必須以明確 ID 或 module factory 啟動這三個 modules，不得解析自然語言。三個 modules 通過後就達成 Phase A exit gate；後續自然語言只進真正的 generative compiler。詳細限制與 `RHB-*`／`GEN-*` requirements 見 [Decision 0002](decisions/0002-reference-harness-boundary.md)。
